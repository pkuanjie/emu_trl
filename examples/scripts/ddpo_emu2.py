...
from trl import DDPOConfig, DDPOEmuTrainer, DefaultDDPOStableDiffusionPipeline, DefaultDDPOEmuPipeline
...
class ScriptArguments:
    pretrained_model: str = field(
        default="/mnt/repos/emu_trl/emu2_ckpts/models--BAAI--Emu2-GEN/snapshots/a41a2dcd777a68225dddc72c7213b064ee06f4a0", metadata={"help": "the pretrained model to use"}
    )
...
def image_outputs_logger(image_data, global_step, accelerate_logger):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for i, image in enumerate(images):
            pil = Image.fromarray(
                    (image.cpu().to(torch.float32).numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
                    )
...
if __name__ == "__main__":
    ...
    ddpo_config.project_kwargs = {
        "logging_dir": "./logs_emu",
        "automatic_checkpoint_naming": True,
        "total_limit": 5,
        "project_dir": "./save_emu",
    }

    pipeline = DefaultDDPOEmuPipeline(args.pretrained_model, use_lora=args.use_lora)

    trainer = DDPOEmuTrainer(
        ddpo_config,
