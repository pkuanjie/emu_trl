...
@dataclass
class DDPOEmu1SchedulerOutput(object):
    """
...
class DDPOEmu1Pipeline(object):
    """
    Main class for the diffusers pipeline of emu to be finetuned with the DDPO trainer
    """

    def __call__(self, *args, **kwargs) -> DDPOEmu1PipelineOutput:
        raise NotImplementedError

    def scheduler_step(self, *args, **kwargs) -> DDPOEmu1SchedulerOutput:
        raise NotImplementedError
...
def scheduler_step(
    ...
    return DDPOEmu1SchedulerOutput(prev_sample.type(sample.dtype), log_prob)
...
class DefaultDDPOEmu1Pipeline(DDPOEmu1Pipeline):
    def __init__(self, pretrained_model_name: str, *, use_lora: bool = True):
        ...
        self.emu1_pipeline.emu_encoder.requires_grad_(False)
    ...
    def save_pretrained(self, output_dir):
        if self.use_lora:
            state_dict = convert_state_dict_to_diffusers(get_peft_model_state_dict(self.emu1_pipeline.unet))
    ...
    def get_trainable_layers(self):
        if self.use_lora:
            ...
            self.emu1_pipeline.unet.add_adapter(lora_config)
