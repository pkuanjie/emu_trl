...
@dataclass
class DDPOEmu1LMMUNetPipelineOutput(object):
    """
...
@dataclass
class DDPOEmu1LMMUNetSchedulerOutput(object):
    """
...
class DDPOEmu1LMMUNetPipeline(object):
    """
    Main class for the diffusers pipeline of emu to be finetuned with the DDPO trainer
    """

    def __call__(self, *args, **kwargs) -> DDPOEmu1LMMUNetPipelineOutput:
        raise NotImplementedError

    def scheduler_step(self, *args, **kwargs) -> DDPOEmu1LMMUNetSchedulerOutput:
        raise NotImplementedError
...
def scheduler_step(
...
) -> DDPOEmu1LMMUNetSchedulerOutput:
    """
    ...
    return DDPOEmu1LMMUNetSchedulerOutput(prev_sample.type(sample.dtype), log_prob)
...
def pipeline_step(
    ...
    return DDPOEmu1LMMUNetPipelineOutput(image, all_latents, all_log_probs), (cross_attention_kwargs,)


class DefaultDDPOEmu1LMMUNetPipeline(DDPOEmu1LMMUNetPipeline):
    def __init__(self, pretrained_model_name: str, *, use_lora: bool = True):
        ...
        self.emu1_pipeline.unet.requires_grad_(not self.use_lora)
    ...
    def get_trainable_layers(self):
        dtype = self.emu1_pipeline.vae.dtype
        device = self.emu1_pipeline.vae.device
        if self.use_lora:
            ...
            self.emu1_pipeline.unet.add_adapter(lora_config_unet)

            # To avoid accelerate unscaling problems in FP16.
            # for param in self.emu1_pipeline.unet.parameters():
            ...
            # if param.requires_grad:
            #     param.data = param.to(torch.float32)
            self.emu1_pipeline.emu_encoder.decoder.lm.model = self.emu1_pipeline.emu_encoder.decoder.lm.model.to(
