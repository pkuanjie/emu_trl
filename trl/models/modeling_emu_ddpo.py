...
class DefaultDDPOEmuPipeline(DDPOEmuPipeline):
    def __init__(self, pretrained_model_name: str, *, use_lora: bool = True):
        ...
        try:
            self.emu_pipeline.load_lora_weights(
        ...
        self.emu_pipeline.scheduler = DDIMScheduler.from_config(self.emu_pipeline.scheduler.config)
        self.emu_pipeline.safety_checker = None

        # memory optimization
        self.emu_pipeline.vae.requires_grad_(False)
    ...
    @property
    def unet(self):
        return self.emu_pipeline.unet

    @property
    def vae(self):
        return self.emu_pipeline.vae

    @property
    def tokenizer(self):
        return self.emu_pipeline.tokenizer

    @property
    def scheduler(self):
        return self.emu_pipeline.scheduler

    @property
    def multimodal_encoder(self):
        return self.emu_pipeline.multimodal_encoder
    ...
    def get_trainable_layers(self):
        if self.use_lora:
            ...
            self.emu_pipeline.unet.add_adapter(lora_config)

            # To avoid accelerate unscaling problems in FP16.
            for param in self.emu_pipeline.unet.parameters():
    ...
    def load_checkpoint(self, models, input_dir):
        if len(models) != 1:
            raise ValueError("Given how the trainable params were set, this should be of length 1")
        if self.use_lora:
            lora_state_dict, network_alphas = self.emu_pipeline.lora_state_dict(
                input_dir, weight_name="pytorch_lora_weights.safetensors"
            )
            self.emu_pipeline.load_lora_into_unet(lora_state_dict, network_alphas=network_alphas, unet=models[0])
